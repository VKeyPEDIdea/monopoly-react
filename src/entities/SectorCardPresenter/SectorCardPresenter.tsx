...
const SectorCardPresenter = ({ currentPlayerId }: SectorCardPresenterProps) => {
  const { id, type, title, color, owner, price, rentPrice, houseList } =
  ...
  const monopolySectorList = useAppSelector((state) =>
    selectMonopolyByColor(state, color || 'blue')
  );
  const housePrice = useAppSelector((state) =>
    getHousePriceBySectorId(state, id)
  );
  const monopolySectorRentPrice = useAppSelector((state) =>
    getSectorRentPrice(state, id)
  );
    ...
    estateList = monopolySectorList.map(({ title, houseList }) => {
      return {
        ...
        buildingList: houseList || [
          { buildType: 'house', price: 0, state: 'vacant' },
        ],
      };
    });
  }
  ...
  const payForExpensiveThing = () => {
    dispatch(
      decreasePlayersCashCount({ playerId: currentPlayerId, count: 200 })
    );
  };

  const payTax = () => {
    dispatch(
      decreasePlayersCashCount({ playerId: currentPlayerId, count: 100 })
    );
  };
  ...
  switch (type) {
    case 'LandPlot':
      card = isMonopoly ? (
        <MonopolyCard
          ...
          isShowToOwner={currentPlayerId === ownerId}
          onPayRentClick={() =>
            payRentMonopolySectorClickHandler({ ...payMonopolyRentPayload })
          }
        ...
        <RealEstateCard
          ...
          onPayRentClick={() =>
            payRentSectorClickHandler({ ...payRentPayload })
          }
        />
      );
      break;
    ...
    case 'Arrest':
      card = (
        <ImageCard
          detailsText="Вы арестованы. Проследуйте в свою камеру"
    ...
    case 'Prison':
      card = (
        <ImageCard
          detailsText="Вы посещаете тюрьму"
          imgSrc="/images/cards/arrested.png"
        />
      );
      break;
    case 'Trap':
      card = (
        <ImageCard
          detailsText="Вы совершили дорогую покупку"
          imgSrc="/images/cards/arrested.png"
        >
          <CardButton
            title="Заплатить 200"
    ...
    case 'Tax':
      card = (
        <ImageCard detailsText="" imgSrc="/images/cards/arrested.png">
          <CardButton title="Заплатить 100" click={() => payTax()} />
        </ImageCard>
      );
      break;
    default:
