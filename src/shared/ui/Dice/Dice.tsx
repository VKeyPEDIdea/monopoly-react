...
const getRotation = (
  ...
  d2: number,
  flip: boolean
): [r1: { x: number; y: number }, r2: { x: number; y: number }] => {
  let r1;
  let r2;
...
const Dice = ({ isActive, onRollDice }: DiceProps) => {
  ...
  const dOneStyle = {
    transform: `rotateX(${r1.x}deg) rotateY(${r1.y}deg) rotateY(${
      flip ? -180 : 0
    }deg)`,
  };
  const dTwoStyle = {
    transform: `rotateX(${r2.x}deg) rotateY(${r2.y}deg) rotateY(${
      flip ? -180 : 0
    }deg)`,
  };
  ...
  return (
    <div className={classes.stage}>
      <div
        className={[
          flip ? classes['group-flip'] : classes.group,
          classes.g1,
        ].join(' ')}
        onClick={rollTheDice}
      >
