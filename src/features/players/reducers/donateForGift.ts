...
const donateForGift = (amount: number, playerId: number) => (dispatch: AppDispatch, getState: () => RootState) => {
    const playersList = getState().players.list.filter(({ id }) => id !== playerId);
