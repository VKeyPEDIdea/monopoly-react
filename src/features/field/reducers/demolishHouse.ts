...
const demolishHouse =
  (sectorId: number) => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    if (sector) {
      const { color } = sector;
      const sectorList: Sector[] = getState().field.sectorList.filter(
        (sector) => sector.color === color
      );

      const lastUpgradedSector = sectorList.find(
      ...
      const prevSector = sectorList[indexOfLastUpgraded - 1]
        ? sectorList[indexOfLastUpgraded - 1]
        : sectorList[sectorList.length - 1];
      demolishHouseIndex = getIndexOfAquiredHouse(
        lastUpgradedSector.houseList!
      );
